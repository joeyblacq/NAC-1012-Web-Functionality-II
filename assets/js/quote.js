...
function calculateElevatorsNeeded() {
  // varible holds residential , commercial , industial
    ...
    if (selectedBuildingType === "residential" &&
        ...
        INPUT_NUMBER_OF_FLOORS.value  &&  INPUT_NUMBER_OF_APARTMENT.value) {
    ...
    // Check if the commercial building type is selected and its fields, number of floors value & maximum occupancy value are filled
     ...
     && INPUT_MAXIMUM_OCCUPANCY.value  ) {
        ...
        elevatorsNeeded =    calculateCommercialElevators(
          INPUT_NUMBER_OF_FLOORS.value ,  INPUT_MAXIMUM_OCCUPANCY.value
        );
    }
    ...
    // Check if the industrial building type is selected and it field, number of elevators value is filled
      ...
      "industrial" && INPUT_NUMBER_OF_ELEVATORS.value ) {
        // Call function that calculates the amount of elevators needed with the proper attributes
...
function calculateCommercialElevators(INPUT_NUMBER_OF_FLOORS, INPUT_MAXIMUM_OCCUPANCY) {
    // Set the varialbe of the total number of occupants
    let TOTAL_NUMBER_OCCUPANTS = Math.ceil(INPUT_MAXIMUM_OCCUPANCY * INPUT_NUMBER_OF_FLOORS);
    ...
    // Set the varialbe of the amount of elevators bank
    let ELEVATOR_BANKS_REQUIRED = Math.ceil(INPUT_NUMBER_OF_FLOORS/10);
    ...
    let ADDITIONAL_ELEVATOR_FOR_FREIGHT_PER_BANK = Math.ceil( ELEVATOR_BANKS_REQUIRED/1 );
...
RADIO_BUTTONS_PRODUCT_LINE.addEventListener('click', calculateElevatorsNeeded)
