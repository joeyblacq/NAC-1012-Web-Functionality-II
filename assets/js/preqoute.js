...
const DIV_NUMBER_OF_APARTMENT = document.getElementById(
  "div-number-of-apartments"
);
...
const DIV_NUMBER_OF_ELEVATORS = document.getElementById(
  "div-number-of-elevators"
);
const DIV_AMOUNT_ELEVATORS_NEEDED = document.getElementById(
  "div-amount-elevators-needed"
);
//
//========================INPUT ELEMENT================================================
...
const INPUT_NUMBER_OF_APARTMENT =
  DIV_NUMBER_OF_APARTMENT.querySelector("input");
...
const INPUT_NUMBER_OF_ELEVATORS =
  DIV_NUMBER_OF_ELEVATORS.querySelector("input");
const INPUT_AMOUNT_ELEVATORS_NEEDED =
  DIV_AMOUNT_ELEVATORS_NEEDED.querySelector("input");

// Set the DOM element variable for group of radio buttons of step 3

const RADIO_BUTTONS_PRODUCT_LINE = document.getElementById(
...
const INPUT_ELEVATOR_UNIT_PRICE = document.getElementById(
  "elevator-unit-price"
);
const INPUT_ELEVATOR_TOTAL_PRICE = document.getElementById(
  "elevator-total-price"
);
...
function showHideFieldOnBuildingType() {
  ...
  if (SELECT_BUILDING_TYPE.value !== "---Select---") {
    ...
    DIV_NUMBER_OF_APARTMENT.style.display = displayResidential
      ? "block"
      : "none";
    DIV_NUMBER_OF_FLOORS.style.display =
      displayCommercial || displayResidential ? "block" : "none";
...
function calculateElevatorsNeeded() {
  ...
  const numberOfFloors = Number(
    document.getElementById("div-number-of-floors").value
  );
  const numberOfApartments = Number(
    document.getElementById("div-number-of-apartments").value
  );
  // const numberOfElevators = Number(document.getElementById("div-amount-elevators-needed").value);

  // const numberOfElevators = Number(document.getElementById("div-number-of-Elevator").value);
  ...
  let calculateElevatorsNeeded = "";

  calculateElevatorsNeeded =
    calculateElevatorsNeeded == null ? String(calculateElevatorsNeeded) : "";
  ...
  function calculateCommercialElevators(
    ...
    let TOTAL_ELEVATORS_REQUIRED_PER_BANK =
      calculateElevatorsNeeded && numberOfElevatorBanks
        ? Math.ceil(elevatorsNeeded / numberOfElevatorBanks)
        : "";
    document.getElementById("INPUT_TOTAL_ELEVATORS_REQUIRED_PER_BANK").value =
      TOTAL_ELEVATORS_REQUIRED_PER_BANK;

    // Set the varialbe of the amount of additional elevators for freight

    let ADDITIONAL_ELEVATOR_FOR_FREIGHT_PER_BANK =
      numberOfFreightElevators && numberOfElevatorBanks
        ? Math.ceil(numberOfFreightElevators / numberOfElevatorBanks)
        : "";
    ...
    return (
      TOTAL_ELEVATORS_REQUIRED_PER_BANK +
      ADDITIONAL_ELEVATOR_FOR_FREIGHT_PER_BANK
    );
  }
  ...
  function calculateInstallationFees(totalPrice, installationPercentFees) {
    if (
      ...
      typeof installationPercentFees !== "number"
    ) {
  ...
  function calculateTotalCost(unitPrice, totalElevatorPrice, installationFees) {
    ...
    if (
      RADIO_BUTTON_STANDARD.checked.checked &&
      !RADIO_BUTTON_PREMIUM.checked &&
      !RADIO_BUTTON_EXCELIUM
    ) {
    ...
    if (
      RADIO_BUTTON_PREMIUM.checked.checked &&
      !RADIO_BUTTON_PREMIUM.checked &&
      !RADIO_BUTTON_EXCELIUM
    ) {
    ...
    if (
      RADIO_BUTTON_EXCELIUM.checked.checked &&
      !RADIO_BUTTON_PREMIUM.checked &&
      !RADIO_BUTTON_EXCELIUM
    ) {
...
RADIO_BUTTONS_PRODUCT_LINE.addEventListener(
  "click",
  showHideFieldOnBuildingType
);
